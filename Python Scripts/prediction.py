...
def getIndicatorRank(targetIndex, allFeatures = 0, toNextYear = 0, rankMethod = 0): 
	"""
	This function returns the ranked list of indicators which have a strong relationship with the target variable.
...
def shapingDataset(folderName, year, targetIndex, indicatorRank):
	"""
	This function creates a numpy array of data X and y for further analysis 
...
def crossValidation(targetIndexInList, allFeatures = 0, toNextYear = 0, rankMethod = 0, numFeatures = 50, algorithm = 'SVR', cv=5):
	"""
	This function evaluates the performance of an algorithm to predict target variable in the next year
...
def getSimilarColIndex(header, testHeader):
	"""
	This function gets indices of the columns in the testHeader which also
